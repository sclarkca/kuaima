...
import com.kuaima.entity.ServiceOrderItem;
...
public class ServiceOrderItemController {
	...
	@DataResolver
	@Transactional
	@Log(module = "服务订单行", category = "CRUD")
	public void save(List<ServiceOrderItem> serviceOrderItems) {
		JpaUtil.save(serviceOrderItems, new SmartSavePolicyAdapter() {
			@Override
			public boolean beforeInsert(SaveContext context) {
				ServiceOrderItem serviceOrderItem = context.getEntity();
				if (EntityState.NEW.equals(EntityUtils.getState(serviceOrderItem))) {
					serviceOrderItem.setCreateDate(new Date());
					serviceOrderItem.setCreater(ContextUtils.getLoginUsername());
				}
				return true;
			}
			...
			public boolean beforeUpdate(SaveContext context) {
				ServiceOrderItem serviceOrderItem = context.getEntity();
				if (EntityState.MODIFIED.equals(EntityUtils.getState(serviceOrderItem))) {
					serviceOrderItem.setUpdateDate(new Date());
					serviceOrderItem.setUpdater(ContextUtils.getLoginUsername());
				}
				return true;
			}
		});
	 ...
	 */
	@DataProvider
	public void query(Page<ServiceOrderItem> page, Map<String, Object> parameter) {
		if (parameter == null) {
	 ...
	 * @param criteria
	 */
	private void buildCondition(Page<ServiceOrderItem> page, Map<String, Object> parameter, Criteria criteria) {
		...
		// 名称
		if (parameter.containsKey("serviceCommdityName")) {
			CriteriaUtils.add(criteria, "serviceCommdityName", FilterOperator.like,
					parameter.get("serviceCommdityName"));
			parameter.remove("serviceCommdityName", parameter.get("serviceCommdityName"));
		}
		// 单位
		if (parameter.containsKey("unit")) {
			CriteriaUtils.add(criteria, "unit", FilterOperator.like, parameter.get("unit"));
			parameter.remove("unit", parameter.get("unit"));
		}
		// 单价
		...
		if (parameter.containsKey("serviceOrderNo")) {
			CriteriaUtils.add(criteria, "price", FilterOperator.like, parameter.get("serviceOrderNo"));
		...
		JpaUtil.linq(ServiceOrderItem.class).where(criteria).paging(page);
	}
	...
	@FileResolver
	@Transactional
	@Log(module = "服务订单行", category = "批量导入")
	...
	public String batchCreateFromExcel(InputStream inputStream) throws Exception {
		...
		List<ServiceOrderItem> newObjectList = new ArrayList<ServiceOrderItem>();
		// 记录错误行
		...
		for (int excelRow = 0; excelRow < rowNum; excelRow++) {
			...
			for (int mappingIdx = 0; mappingIdx < headerList.size(); mappingIdx++) {
				String propertyType = ServiceOrderItem.getPropertyTypeByEnum(initMappingList.get(mappingIdx));
				String propertyName = ServiceOrderItem.getPropertyNameByEnum(initMappingList.get(mappingIdx));
				if (initMappingList.get(mappingIdx) != 0) {
					...
					if (propertyType.equals("String")) {
						...
						BeanUtils.setProperty(serviceOrderItem, propertyName, arg);
					}
					if (propertyType.equals("enum")) {
						...
						if (StringUtils.isBlank(valueFromForm)) {
							...
							// 不必填
							if (!isRequiredByRelatedFieldValue(propertyName)) {
								BeanUtils.setProperty(serviceOrderItem, propertyName, 0);
							}
						}
					...
					if (propertyType.equals("table")) {
						...
						if (StringUtils.isNotBlank(valueFromForm)) {
							...
							// TABLE能查到值
							if (!valueFromString.equals("error")) {
								BeanUtils.setProperty(serviceOrderItem, propertyName, valueFromString);
							}
						}
					}
					...
					if (propertyType.equals("date")) {
						...
						if (StringUtils.isBlank(valueFromForm)) {
							...
							if (!isRequiredByRelatedFieldValue(propertyName)) {
								/*
								 * BeanUtils.setProperty(serviceOrderItem,
								 * propertyName, null);
								 */
							}
						}
		...
		List<ServiceOrderItem> goodObjectList = new ArrayList<ServiceOrderItem>();
		for (int i = 0; i < goodIdxList.size(); i++) {
			ServiceOrderItem serviceOrderItem = newObjectList.get(goodIdxList.get(i));
			if (!MyBeanUtil.checkAllFieldValueNull(serviceOrderItem)) {
				goodObjectList.add(serviceOrderItem);
			}
		}
		...
		if (goodExcelDataList.size() > 0) {
			for (int i = 0; i < goodObjectList.size(); i++) {
				// 创建动作
				ServiceOrderItem serviceOrderItem = goodObjectList.get(i);
				serviceOrderItem.setCreateDate(new Date());
				serviceOrderItem.setCreater(ContextUtils.getLoginUsername());
				JpaUtil.persist(serviceOrderItem);
			}
		}
