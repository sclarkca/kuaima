...
import com.kuaima.entity.Logistics;
...
public class LogisticsController {
	// 物流
	private static final String ENCODING_TYPE_CODE = "LOGISTICS";
	// 物流编码前缀
	...
	@DataResolver
	@Transactional
	@Log(module = "物流公司", category = "CRUD")
	public void save(List<Logistics> logisticss) {
		JpaUtil.save(logisticss, new SmartSavePolicyAdapter() {
			@Override
			public boolean beforeInsert(SaveContext context) {
				Logistics logistics = context.getEntity();
				if (EntityState.NEW.equals(EntityUtils.getState(logistics))) {
					logistics.setCreateDate(new Date());
					logistics.setCreater(ContextUtils.getLoginUsername());
					String logisticsCompanyCode = logistics.getLogisticsCompanyCode();
					this.crudCode(logisticsCompanyCode);
				}
				return true;
			}
			...
			public boolean beforeUpdate(SaveContext context) {
				Logistics logistics = context.getEntity();
				if (EntityState.MODIFIED.equals(EntityUtils.getState(logistics))) {
					logistics.setUpdateDate(new Date());
					logistics.setUpdater(ContextUtils.getLoginUsername());
					String logisticsCompanyCode = logistics.getLogisticsCompanyCode();
					this.crudCode(logisticsCompanyCode);
				}
				return true;
	 ...
	 */
	@DataProvider
	public void query(Page<Logistics> page, Map<String, Object> parameter) {
		if (parameter == null) {
	 ...
	 * @param criteria
	 */
	private void buildCondition(Page<Logistics> page, Map<String, Object> parameter, Criteria criteria) {
		...
		// 物流公司名称
		if (parameter.containsKey("logisticsName")) {
			CriteriaUtils.add(criteria, "logisticsName", FilterOperator.like, parameter.get("logisticsName"));
			parameter.remove("logisticsName", parameter.get("logisticsName"));
		}
		MyCriteriaUtils.buildCriteriaMap(parameter);
		criteria = MyCriteriaUtils.add(criteria, parameter);
		JpaUtil.linq(Logistics.class).where(criteria).paging(page);
	}
	...
	public String batchCreateFromExcel(InputStream inputStream) throws Exception {
		...
		List<Logistics> newObjectList = new ArrayList<Logistics>();
		// 记录错误行
		...
		for (int excelRow = 0; excelRow < rowNum; excelRow++) {
			...
			for (int mappingIdx = 0; mappingIdx < headerList.size(); mappingIdx++) {
				String propertyType = Logistics.getPropertyTypeByEnum(initMappingList.get(mappingIdx));
				String propertyName = Logistics.getPropertyNameByEnum(initMappingList.get(mappingIdx));
				if (initMappingList.get(mappingIdx) != 0) {
					...
					if (propertyType.equals("String")) {
						...
						BeanUtils.setProperty(logistics, propertyName, arg);
					}
					if (propertyType.equals("enum")) {
						...
						if (StringUtils.isBlank(valueFromForm)) {
							...
							// 不必填
							if (!isRequiredByRelatedFieldValue(propertyName)) {
								BeanUtils.setProperty(logistics, propertyName, 0);
							}
						}
					...
					if (propertyType.equals("table")) {
						...
						if (StringUtils.isNotBlank(valueFromForm)) {
							...
							// TABLE能查到值
							if (!valueFromString.equals("error")) {
								BeanUtils.setProperty(logistics, propertyName, valueFromString);
							}
						}
					}
					...
					if (propertyType.equals("date")) {
						...
						if (StringUtils.isBlank(valueFromForm)) {
							...
							if (!isRequiredByRelatedFieldValue(propertyName)) {
								/*
								 * BeanUtils.setProperty(logistics,
								 * propertyName, null);
								 */
							}
						}
		...
		List<Logistics> goodObjectList = new ArrayList<Logistics>();
		for (int i = 0; i < goodIdxList.size(); i++) {
			Logistics logistics = newObjectList.get(goodIdxList.get(i));
			if (!MyBeanUtil.checkAllFieldValueNull(logistics)) {
				goodObjectList.add(logistics);
			}
		}
		...
		if (goodExcelDataList.size() > 0) {
			for (int i = 0; i < goodObjectList.size(); i++) {
				// 创建动作
				Logistics logistics = goodObjectList.get(i);
				logistics.setCreateDate(new Date());
				logistics.setCreater(ContextUtils.getLoginUsername());
				JpaUtil.persist(logistics);
			}
		}
	 ...
	 */
	@DataProvider
	public List<Logistics> queryWuliuList(){
		return JpaUtil.linq(Logistics.class).list();
	}
}