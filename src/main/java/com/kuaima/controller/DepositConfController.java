...
import com.kuaima.entity.DepositConf;
...
public class DepositConfController {
	/**
	 * 增删改
	 */
	@DataResolver
	@Transactional
	@Log(module = "押金配置", category = "CRUD")
	public void save(List<DepositConf> depositConfs) {
		JpaUtil.save(depositConfs, new SmartSavePolicyAdapter() {
			@Override
			public boolean beforeInsert(SaveContext context) {
				DepositConf depositConf = context.getEntity();
				if (EntityState.NEW.equals(EntityUtils.getState(depositConf))) {
					depositConf.setCreateDate(new Date());
					depositConf.setCreater(ContextUtils.getLoginUsername());
				}
				return true;
			}
			...
			public boolean beforeUpdate(SaveContext context) {
				DepositConf depositConf = context.getEntity();
				if (EntityState.MODIFIED.equals(EntityUtils.getState(depositConf))) {
					depositConf.setUpdateDate(new Date());
					depositConf.setUpdater(ContextUtils.getLoginUsername());
				}
				return true;
			}
		});
	 ...
	 */
	@DataProvider
	public void query(Page<DepositConf> page, Map<String, Object> parameter) {
		if (parameter == null) {
	 ...
	 * @param criteria
	 */
	private void buildCondition(Page<DepositConf> page, Map<String, Object> parameter, Criteria criteria) {
		...
		// 押金额度
		if (parameter.containsKey("depositQuota")) {
			CriteriaUtils.add(criteria, "depositQuota", FilterOperator.like, parameter.get("depositQuota"));
			parameter.remove("depositQuota", parameter.get("depositQuota"));
		}
		// 押金类型
		if (parameter.containsKey("depositType")) {
			CriteriaUtils.add(criteria, "depositType", FilterOperator.like, parameter.get("depositType"));
			parameter.remove("depositType", parameter.get("depositType"));
		}
		MyCriteriaUtils.buildCriteriaMap(parameter);
		criteria = MyCriteriaUtils.add(criteria, parameter);
		JpaUtil.linq(DepositConf.class).where(criteria).paging(page);
	}
	...
	public String batchCreateFromExcel(InputStream inputStream) throws Exception {
		...
		List<DepositConf> newObjectList = new ArrayList<DepositConf>();
		// 记录错误行
		...
		for (int excelRow = 0; excelRow < rowNum; excelRow++) {
			...
			// 入库实体类
			DepositConf depositConf = new DepositConf();
			// 记录错误类
			...
			for (int mappingIdx = 0; mappingIdx < headerList.size(); mappingIdx++) {
				String propertyType = DepositConf.getPropertyTypeByEnum(initMappingList.get(mappingIdx));
				String propertyName = DepositConf.getPropertyNameByEnum(initMappingList.get(mappingIdx));
				if (initMappingList.get(mappingIdx) != 0) {
					...
					if (propertyType.equals("String")) {
						...
						BeanUtils.setProperty(depositConf, propertyName, arg);
					}
					if (propertyType.equals("enum")) {
						...
						if (StringUtils.isBlank(valueFromForm)) {
							...
							// 不必填
							if (!isRequiredByRelatedFieldValue(propertyName)) {
								BeanUtils.setProperty(depositConf, propertyName, 0);
							}
						}
					...
					if (propertyType.equals("table")) {
						...
						if (StringUtils.isNotBlank(valueFromForm)) {
							...
							// TABLE能查到值
							if (!valueFromString.equals("error")) {
								BeanUtils.setProperty(depositConf, propertyName, valueFromString);
							}
						}
					}
					...
					if (propertyType.equals("date")) {
						...
						if (StringUtils.isBlank(valueFromForm)) {
							...
							if (!isRequiredByRelatedFieldValue(propertyName)) {
								/*
								 * BeanUtils.setProperty(depositConf,
								 * propertyName, null);
								 */
							}
						}
		...
		List<DepositConf> goodObjectList = new ArrayList<DepositConf>();
		for (int i = 0; i < goodIdxList.size(); i++) {
			DepositConf depositConf = newObjectList.get(goodIdxList.get(i));
			if (!MyBeanUtil.checkAllFieldValueNull(depositConf)) {
				goodObjectList.add(depositConf);
			}
		}
		...
		if (goodExcelDataList.size() > 0) {
			for (int i = 0; i < goodObjectList.size(); i++) {
				// 创建动作
				DepositConf depositConf = goodObjectList.get(i);
				depositConf.setCreateDate(new Date());
				depositConf.setCreater(ContextUtils.getLoginUsername());
				JpaUtil.persist(depositConf);
			}
		}
