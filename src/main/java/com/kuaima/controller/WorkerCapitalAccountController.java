...
import com.kuaima.entity.WorkerCapitalAccount;
...
public class WorkerCapitalAccountController {
	/**
	 * 增删改
	 */
	@DataResolver
	@Transactional
	@Log(module = "师傅资金账户", category = "CRUD")
	public void save(List<WorkerCapitalAccount> workerCapitalAccounts) {
		JpaUtil.save(workerCapitalAccounts, new SmartSavePolicyAdapter() {
			@Override
			public boolean beforeInsert(SaveContext context) {
				WorkerCapitalAccount workerCapitalAccount = context.getEntity();
				if (EntityState.NEW.equals(EntityUtils.getState(workerCapitalAccount))) {
					workerCapitalAccount.setCreateDate(new Date());
					workerCapitalAccount.setCreater(ContextUtils.getLoginUsername());
				}
				return true;
			}
			...
			public boolean beforeUpdate(SaveContext context) {
				WorkerCapitalAccount workerCapitalAccount = context.getEntity();
				if (EntityState.MODIFIED.equals(EntityUtils.getState(workerCapitalAccount))) {
					workerCapitalAccount.setUpdateDate(new Date());
					workerCapitalAccount.setUpdater(ContextUtils.getLoginUsername());
				}
				return true;
			}
		});
	 ...
	 */
	@DataProvider
	public void query(Page<WorkerCapitalAccount> page, Map<String, Object> parameter) {
		if (parameter == null) {
	 ...
	 * @param criteria
	 */
	private void buildCondition(Page<WorkerCapitalAccount> page, Map<String, Object> parameter, Criteria criteria) {
		...
		// 可提现金额
		if (parameter.containsKey("canCashAmount")) {
			CriteriaUtils.add(criteria, "canCashAmount", FilterOperator.like, parameter.get("canCashAmount"));
			parameter.remove("canCashAmount", parameter.get("canCashAmount"));
		}
		// 冻结金额
		if (parameter.containsKey("frozenAmount")) {
			CriteriaUtils.add(criteria, "frozenAmount", FilterOperator.like, parameter.get("frozenAmount"));
			parameter.remove("frozenAmount", parameter.get("frozenAmount"));
		}
		// 已提现总额
		if (parameter.containsKey("alreadyCashAmount")) {
			CriteriaUtils.add(criteria, "alreadyCashAmount", FilterOperator.like, parameter.get("alreadyCashAmount"));
			parameter.remove("alreadyCashAmount", parameter.get("alreadyCashAmount"));
		}
		// 师傅编码
		if (parameter.containsKey("serverId")) {
			CriteriaUtils.add(criteria, "serverId", FilterOperator.like, parameter.get("serverId"));
			parameter.remove("serverId", parameter.get("serverId"));
		}
		MyCriteriaUtils.buildCriteriaMap(parameter);
		criteria = MyCriteriaUtils.add(criteria, parameter);
		JpaUtil.linq(WorkerCapitalAccount.class).where(criteria).paging(page);
	}
	...
	@FileResolver
	@Transactional
	@Log(module = "师傅资金账户", category = "批量导入")
	...
	public String batchCreateFromExcel(InputStream inputStream) throws Exception {
		...
		List<WorkerCapitalAccount> newObjectList = new ArrayList<WorkerCapitalAccount>();
		// 记录错误行
		...
		for (int excelRow = 0; excelRow < rowNum; excelRow++) {
			...
			// 入库实体类
			WorkerCapitalAccount workerCapitalAccount = new WorkerCapitalAccount();
			// 记录错误类
			...
			for (int mappingIdx = 0; mappingIdx < headerList.size(); mappingIdx++) {
				String propertyType = WorkerCapitalAccount.getPropertyTypeByEnum(initMappingList.get(mappingIdx));
				String propertyName = WorkerCapitalAccount.getPropertyNameByEnum(initMappingList.get(mappingIdx));
				if (initMappingList.get(mappingIdx) != 0) {
					...
					if (propertyType.equals("String")) {
						...
						BeanUtils.setProperty(workerCapitalAccount, propertyName, arg);
					}
					if (propertyType.equals("enum")) {
						...
						if (StringUtils.isBlank(valueFromForm)) {
							...
							// 不必填
							if (!isRequiredByRelatedFieldValue(propertyName)) {
								BeanUtils.setProperty(workerCapitalAccount, propertyName, 0);
							}
						}
					...
					if (propertyType.equals("table")) {
						...
						if (StringUtils.isNotBlank(valueFromForm)) {
							...
							// TABLE能查到值
							if (!valueFromString.equals("error")) {
								BeanUtils.setProperty(workerCapitalAccount, propertyName, valueFromString);
							}
						}
					}
					...
					if (propertyType.equals("date")) {
						...
						if (StringUtils.isBlank(valueFromForm)) {
							...
							if (!isRequiredByRelatedFieldValue(propertyName)) {
								/*
								 * BeanUtils.setProperty(workerCapitalAccount,
								 * propertyName, null);
								 */
							}
						}
		...
		List<WorkerCapitalAccount> goodObjectList = new ArrayList<WorkerCapitalAccount>();
		for (int i = 0; i < goodIdxList.size(); i++) {
			WorkerCapitalAccount workerCapitalAccount = newObjectList.get(goodIdxList.get(i));
			if (!MyBeanUtil.checkAllFieldValueNull(workerCapitalAccount)) {
				goodObjectList.add(workerCapitalAccount);
			}
		}
		...
		if (goodExcelDataList.size() > 0) {
			for (int i = 0; i < goodObjectList.size(); i++) {
				// 创建动作
				WorkerCapitalAccount workerCapitalAccount = goodObjectList.get(i);
				workerCapitalAccount.setCreateDate(new Date());
				workerCapitalAccount.setCreater(ContextUtils.getLoginUsername());
				JpaUtil.persist(workerCapitalAccount);
			}
		}
