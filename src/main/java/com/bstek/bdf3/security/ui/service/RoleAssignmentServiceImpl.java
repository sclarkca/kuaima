...
public class RoleAssignmentServiceImpl implements RoleAssignmentService {
    ...
    public void loadUsersWithout(Page<User> page, Criteria criteria, String roleId) {
        JpaUtil.linq(User.class).where(criteria).notExists(RoleGrantedAuthority.class)
                .equalProperty("actorId", "username").equal("roleId", roleId).end().paging(page);
        ;
    }

    @Override
    public void loadUsersWithin(Page<User> page, Criteria criteria, String roleId) {
        JpaUtil.linq(User.class).where(criteria).exists(RoleGrantedAuthority.class).equalProperty("actorId", "username")
                .equal("roleId", roleId).end().paging(page);
        ;
    }
    ...
    private void anonymousMethod(List<GrantedAuthority> authorities) {
    	if (authorities != null) {
